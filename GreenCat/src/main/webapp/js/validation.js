...
BUZZWORDS.namespace('BUZZWORDS.validation.validateBirthDate');
BUZZWORDS.namespace('BUZZWORDS.validation.validatePhone');
...
BUZZWORDS.namespace('BUZZWORDS.validation.runFormValidators');
...
$(document).ready(function() {
     ...
     */
    BUZZWORDS.validation.displayServerSideErrors = function (pageID, errorMsg) {
    ...
    $('.birthDate').attr('max', function() {
        return BUZZWORDS.util.getCurrentDate();
    });

    /**
     * Check if the birthdate fits the regex pattern of YYYY-DD-MM.
     */ 
    BUZZWORDS.validation.validateBirthDate = function(date) {
    ...
    $.validator.addMethod("birthdate", function(value, element) {
        return this.optional(element) || BUZZWORDS.validation.validateBirthDate(value);
     ...
     */
    BUZZWORDS.validation.validatePhone = function(phone, divID) {
...
//        }
    ...
    $.validator.addMethod("intlTel", function(value, element, param) {
        return this.optional(element) || BUZZWORDS.validation.validatePhone(value, param);
    }, "Please use a valid phone number with county code.");
    
    /**
     * Check if the name fits the regex pattern of a Alpha characters only.
     */
    BUZZWORDS.validation.validateName = function(name) {
    ...
    $.validator.addMethod("personName", function(value, element) {
        return this.optional(element) || BUZZWORDS.validation.validateName(value);
     ...
     */
    BUZZWORDS.validation.validateEmailUniqueness = function(email) {
        if (BUZZWORDS.validation.formEmail === email){
            return false;
        } else {
            return true;
        }
    ...
    $.validator.addMethod("emailUnique", function(value, element) {
        return this.optional(element) || BUZZWORDS.validation.validateEmailUniqueness(value);
     ...
     */ 
    BUZZWORDS.validation.runFormValidators = function() {
        // Turn on the intl-Tel-Input validation for the Add form. Turn it on for the Edit form in the edit.js buildBuzzwordDetail.
        $('#buzzwords-add-input-tel').intlTelInput({nationalMode:false});
        ...
        BUZZWORDS.validation.addBuzzwordsFormValidator = $('#buzzwords-add-form').validate({
            rules: {
                firstName: {
                    required: true,
...
//                    phoneUS: true
                ...
                email: {
                    required: true,
                    email: true,
                    // This is the custom validator created above to make sure that the email is not being used in another buzzword.
                    emailUnique: true
                },
                birthDate: {
                    required: true,
...
//                    phoneUS: "Please use a standard US formats. And remember the area code and prefix may not start with 1."
         ...
         *       It appears that the plugin only works when it is given only 1 form at a time. 
         */
        BUZZWORDS.validation.editBuzzwordsFormValidator = $('#buzzwords-edit-form').validate({
            rules: {
                firstName: {
                    required: true,
                    // This is the custom validator created above to make sure that the String only has letter, -, or '.
                ...
                phoneNumber: {
                    required: true,
                    intlTel: {
                    	param: "#buzzwords-edit-input-tel"
                    }
//                    phoneUS: true
                },
                email: {
...
//                    phoneUS: "Please use a standard US formats. And remember the area code and prefix may not start with 1."
     ...
     */
    BUZZWORDS.validation.runFormValidators();
}); 
